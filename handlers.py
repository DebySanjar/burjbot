...
from states import FormStates, AdminStates
from keyboards import (
    main_menu, cancel_keyboard, confirm_keyboard, admin_keyboard,
    phone_keyboard, search_filter_keyboard
)
import database
...
async def cmd_start(message: Message):
    ...
    is_admin_mode = admin_mode.get(user_id, True) if user_id == ADMIN_ID else False
    
    if user_id == ADMIN_ID and is_admin_mode:
...
async def switch_to_user_mode(message: Message):
    """Admin foydalanuvchi rejimiga o'tadi"""
...
async def switch_to_admin_mode(message: Message):
    ...
    admin_mode[ADMIN_ID] = True
    await message.answer("✅ Admin rejimiga qaytdingiz!", reply_markup=admin_keyboard())
...
async def process_age(message: Message, state: FSMContext):
    ...
    # Yosh faqat raqam ekanligini tekshirish
    if not message.text.isdigit():
        await message.answer("❌ Yosh faqat raqam bo'lishi kerak!\n\nYoshingizni qayta kiriting:", reply_markup=cancel_keyboard())
        return
    
    age = int(message.text)
    if age < 16 or age > 70:
        await message.answer("❌ Yosh 16 dan 70 gacha bo'lishi kerak!\n\nYoshingizni qayta kiriting:", reply_markup=cancel_keyboard())
        return
...
async def admin_vacancy(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📝 Yangi vakansiya matnini yuboring:\n\nBu matn barcha foydalanuvchilarga yuboriladi.",
        reply_markup=cancel_keyboard()
    )

@router.message(AdminStates.vacancy_text)
async def process_vacancy(message: Message, state: FSMContext, bot):
    if message.text == "❌ Bekor qilish":
        await state.clear()
        await message.answer("❌ Bekor qilindi.", reply_markup=admin_keyboard())
        return
    
    database.set_vacancy(message.text)
    user_ids = database.get_all_user_ids()
    success_count = 0
    failed_count = 0
    
    await message.answer("⏳ Yuborilmoqda...", reply_markup=admin_keyboard())
